...
class SqliteProductRepository(ProductRepository):
    ...
    # ---------- queries (public) ----------
    def list_products_public_view(self, search_query: Optional[str] = None) -> list[dict]:
        ...
        search_clause = ""
        if search_query:
            search_clause = "AND p.display_name LIKE ?"
            params.append(f"%{search_query}%")
            
        sql = sql.replace("{search_clause}", search_clause)
        
        with self._conn() as conn:
            return [dict(r) for r in conn.execute(sql, params).fetchall()]

    def get_product_aggregated_price_history(
        self, product_id: UUID
    ) -> list[dict]:
        """Lowest price per day across all auto/confirmed listings of a product."""
        sql = """
        SELECT date(ps.fetched_at) AS day, MIN(ps.price) AS lowest_price
        FROM listings l
        JOIN price_snapshots ps ON ps.listing_id = l.id
        WHERE l.product_id = ?
          AND l.link_status IN ('auto', 'confirmed')
